...
class LoginWithAjax {
  ...
  public static function init(){
    ...
    if ( !empty($_REQUEST["lwa"]) ) { //AJAX Request
      ...
      if( !is_admin() ) {
          $js_url = defined('WP_DEBUG') && WP_DEBUG ? 'login-with-ajax.source.js':'login-with-ajax.source.js';
  ...
  public static function register(){
    $return = array();
    if( !function_exists('login_with_ajax_register_new_user') ){
        include_once('registration.php'); //in ajax we don't have access to this function, so include our own copy of the function
    }
    $errors = login_with_ajax_register_new_user($_POST['user_login'], $_POST['user_email']);
